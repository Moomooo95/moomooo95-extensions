...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AstralManga = exports.AstralMangaInfo = void 0;
const types_1 = require("@paperback/types");
const Madara_1 = require("../templates/Madara/Madara");
const DOMAIN = 'https://astral-manga.fr';
exports.AstralMangaInfo = {
    version: "1.0",
    language: "FR",
    name: 'AstralManga',
    icon: 'icon.png',
...
class AstralManga extends Madara_1.Madara {
    base_url = DOMAIN;
    lang_code = exports.AstralMangaInfo.language;
    alt_ajax = true;
    description_selector = "div.manga-excerpt p";
    genres_condition_filter_or = "OU (avoir un des genres sélectionnés)";
    genres_condition_filter_and = "ET (avoir tous les genres sélectionnés)";
    adult_filter_all = "Tous";
    adult_filter_none = "Aucun contenu adulte";
    adult_filter_only = "Contenu pour adultes uniquement";
    ...
    status_filter_upcoming = "Prochainement";
}
exports.AstralManga = AstralManga;
...
class Madara {
    ...
    chapter_selector = "li.wp-manga-chapter";
    chapter_details_selector = "div.page-break > img";
    alt_ajax = false;
    viewer = ($, categories) => {
    ...
    genres_condition_filter_or = "OR (having one of selected genres)";
    genres_condition_filter_and = "AND (having all selected genres)";
    adult_filter_all = "All";
    ...
    status_filter_cancelled = "Canceled";
    status_filter_on_hold = "On Hold";
    status_filter_upcoming = "Upcoming";
    genres_filter_string = "Genres";
    genres_condition_filter_string = "Genres Condition";
    adult_filter_string = "Adult Content";
    statut_filter_string = "Statut";
    ...
    async getMangaDetails(mangaId) {
        ...
        return (0, MadaraParser_1.parseMangaDetails)($, mangaId, this);
    }
    ///////////////////////////////////
    /////    CHAPTER PROVIDING    /////
    ///////////////////////////////////
    async getChapters(mangaId) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, MadaraParser_1.parseChapters)($, mangaId, this);
    }
    async getChapterDetails(mangaId, chapterId) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, MadaraParser_1.parseChapterDetails)($, mangaId, chapterId, this);
    }
    //////////////////////////////////////////
    ...
    async getSearchResults(query, metadata) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        const manga = (0, MadaraParser_1.parseSearchResults)($, this);
    ...
    async getSearchTags() {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return await (0, MadaraParser_1.parseSearchTags)($, this);
    }
    async getSearchFields() {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return await (0, MadaraParser_1.parseSearchFields)($);
    }
    /////////////////////////////////////////////
    ...
    async getHomePageSections(sectionCallback) {
        ...
        for (const section of sections) {
            ...
            const response = await this.requestManager.schedule(request, 1);
            const $ = this.cheerio.load(response.data);
            section.items = (0, MadaraParser_1.parseHomePageSections)($, this);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        const $ = this.cheerio.load(response.data);
        return await App.createPagedResults({
            results: (0, MadaraParser_1.parseViewMoreItems)($, homepageSectionId, this),
...
},{"./MadaraParser":67,"@paperback/types":61}],66:[function(require,module,exports){
"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getImageUrl = exports.parseDate = exports.decodeHtmlEntity = void 0;
const moment_1 = __importDefault(require("moment"));
const fr_1 = __importDefault(require("moment/locale/fr"));
moment_1.default.updateLocale('fr', fr_1.default);
...
function parseDate(date_str, date_format) {
    date_str = decodeHtmlEntity(date_str);
    let date = (0, moment_1.default)(date_str, date_format);
    ...
    if (match) {
        ...
        if (unit == "s")
            unit = "w";
        date = (0, moment_1.default)().subtract(match[1], unit);
    }
    if (!date.isValid()) {
        date = (0, moment_1.default)().startOf("day");
    }
    return date.toDate();
}
...
},{"moment":63,"moment/locale/fr":62}],67:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
const parseMangaDetails = async ($, mangaId, data) => {
    const titles = [(0, MadaraHelper_1.decodeHtmlEntity)($('div.post-title h1').text().trim())];
    const image = (0, MadaraHelper_1.getImageUrl)($, $('div.summary_image img'));
    ...
    const description = (0, MadaraHelper_1.decodeHtmlEntity)($(data.description_selector).text().trim());
...
const parseChapters = ($, mangaId, data) => {
    ...
    for (const chapter of $(data.chapter_selector).toArray()) {
        ...
        const date = (0, MadaraHelper_1.parseDate)($('span.chapter-release-date', chapter).text().trim(), data.date_format);
...
const parseChapterDetails = async ($, mangaId, chapterId, data) => {
    ...
    for (const page of $(data.chapter_details_selector).toArray()) {
        pages.push((0, MadaraHelper_1.getImageUrl)($, page));
    }
...
const parseSearchResults = ($, data) => {
    ...
    for (const manga of $(data.search_selector).toArray()) {
        ...
        const image = (0, MadaraHelper_1.getImageUrl)($, $('img', manga));
        const title = (0, MadaraHelper_1.decodeHtmlEntity)($('h3 a', manga).text()) ?? '';
        const subtitle = (0, MadaraHelper_1.decodeHtmlEntity)($('.latest-chap .chapter a', manga).text());
...
const parseSearchTags = ($, data) => {
    ...
    for (let item of $('#search-advanced .checkbox-group .checkbox').toArray()) {
        ...
        let label = (0, MadaraHelper_1.decodeHtmlEntity)($('label', item).text().trim());
    ...
    for (let item of $('#search-advanced .form-group .form-control').eq(0).children().toArray()) {
        ...
        let label = (0, MadaraHelper_1.decodeHtmlEntity)($(item).text().trim());
    ...
    for (let item of $('#search-advanced .form-group').eq(6).children('.checkbox-inline').toArray()) {
        let id = $('input', item).attr('value') ?? '';
        let label = (0, MadaraHelper_1.decodeHtmlEntity)($('label', item).text().trim().replace(/([\u2700-\u27BF]|[\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2011-\u26FF]|\uD83E[\uDD10-\uDDFF])/g, ''));
...
const parseViewMoreItems = ($, homepageSectionId, data) => {
    const mangaItems = [];
    const collectedIds = [];
    for (const manga of $("div.page-item-detail").toArray()) {
        const id = $(data.base_id_selector, manga).attr('href')?.split('/')[4] ?? '';
        const image = (0, MadaraHelper_1.getImageUrl)($, $('img', manga));
        const title = (0, MadaraHelper_1.decodeHtmlEntity)($(data.base_id_selector, manga).text()) ?? '';
        if (!id || !title || collectedIds.includes(id))
            continue;
        mangaItems.push(App.createPartialSourceManga({
            image: image,
